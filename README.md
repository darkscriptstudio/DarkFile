# Dark File Tool v1.0 (Beta Version)
...
**Dark File Tool** is a high-performance, multi-threaded file management utility designed for maximum speed and efficiency. Written in pure C, it leverages **Zero-Copy (`sendfile`)** technology and intelligent thread pooling to handle massive file operations significantly faster than traditional system tools.
...
```bash
gcc src/darkfile.c -o darkfile -pthread -O3
```

#### Windows (MinGW)

```bash
gcc src/darkfile.c -o darkfile.exe -pthread -O3
```
